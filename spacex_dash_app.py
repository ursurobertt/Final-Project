...
def get_pie_chart(enterered_site):
    filtered_df = spacex_df
    if enterered_site == 'ALL':
        fig = px.pie(spacex_df, values='class', names='Launch Site', title='Total Success Launches By Site')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == enterered_site]
        fig = px.pie(filtered_df, names='class', title='Total Success Launches for site ' + enterered_site)
        return fig
# TASK 4:
...
def get_scatter_chart(enterered_site, payload_range):
    filtered_df = spacex_df
    if enterered_site == 'ALL':
        ...
        filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= payload_range[0]) & (spacex_df['Payload Mass (kg)'] <= payload_range[1]) & (spacex_df['Launch Site'] == enterered_site)]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Correlation between Payload and Success for site ' + enterered_site)
        return fig
